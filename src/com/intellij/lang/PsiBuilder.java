...
public interface PsiBuilder extends UserDataHolder, UserDataHolderUnprotected {
  /**
   * Returns a project for which PSI builder was created (see { PsiBuilderFactory}).
   * @return project.
   */
   ...
   *  #setTokenTypeRemapper(ITokenTypeRemapper).
   */
  @Nullable
  ...
  interface Marker {
    ...
    /**
     * Like {plain #done(IElementType)}, but collapses all tokens between start and end markers
    ...
    /**
     * Like {plain #done(IElementType)}, but the marker is completed (end marker inserted)
    ...
    /**
     * Like {plain #doneBefore(IElementType, Marker)}, but in addition an error element with given text
    ...
    /**
     * Like {plain #error(String)}, but the marker is completed before specified one.
     *
     * @param message for error element.
     * @param before  marker to complete this one before.
     */
  ...
  /**
   * Same as { #getTreeBuilt()} but returns a light tree, which is build faster,
   * produces less garbage but is incapable of creating a PSI over.
   * <br><b>Note</b>: this method shouldn't be called if { #getTreeBuilt()} was called before.
   *
