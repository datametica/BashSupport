...
public interface PsiElement extends UserDataHolder {
   ...
   * and not yet committed ({ com.intellij.psi.PsiDocumentManager#commitDocument(com.intellij.openapi.editor.Document)}).
   * (In this case an attempt to change PSI will result in an exception).
   *
   * Any access to invalid elements results in { com.intellij.psi.PsiInvalidElementAccessException}.
   *
   * Once invalid, elements can't become valid again.
   *
   * Elements become invalid in following cases:
   * <ul>
   *   <li>They have been deleted via PSI operation ({ #delete()})</li>
   ...
   * multiple possible targets, { PsiPolyVariantReference} should be used instead
   * of returning multiple references.
   * <p/>
   * Actually, it's preferable to call { com.intellij.psi.PsiReferenceService#getReferences} instead
   * as it allows adding references by plugins when the element implements { com.intellij.psi.ContributedReferenceHost}.
   *
   * @return the array of references, or an empty array if the element has no associated
   *         references.
   *  com.intellij.psi.PsiReferenceService#getReferences
   *  com.intellij.psi.search.searches.ReferencesSearch
   */
  @NotNull
  @Contract(pure=true)
   ...
   * Also, { PsiDocumentManager#getDocument(PsiFile)} returns null for non-physical elements.
   ...
   *  { com.intellij.psi.search.PsiSearchHelper#getUseScope(PsiElement)}
   */
  @NotNull
  @Contract(pure=true)
