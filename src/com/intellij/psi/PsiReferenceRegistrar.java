 ...
 * { com.intellij.patterns.ElementPattern}s. If a pattern matches some PSI element, then the corresponding
 * { PsiReferenceProvider#getReferencesByElement(PsiElement, com.intellij.util.ProcessingContext)} is executed, from
 * which one can return the references whose { PsiReference#getElement()} is the same as the first parameter of
...
public abstract class PsiReferenceRegistrar {
  ...
  /**
   * Register reference provider with default priority ({ #DEFAULT_PRIORITY})
   * @param pattern reference place description. See { com.intellij.patterns.StandardPatterns}, { com.intellij.patterns.PlatformPatterns} and their extenders
   ...
   * @param pattern reference place description. See { com.intellij.patterns.StandardPatterns}, { com.intellij.patterns.PlatformPatterns} and their extenders
   * @param provider provider to be registered
   * @param priority  DEFAULT_PRIORITY, HIGHER_PRIORITY, LOWER_PRIORITY
   */
