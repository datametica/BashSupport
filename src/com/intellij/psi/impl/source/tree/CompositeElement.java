...
public class CompositeElement extends TreeElement {
  ...
  public void addLeaf(@NotNull final IElementType leafType, final CharSequence leafText, final ASTNode anchorBefore) {
    FileElement holder = new DummyHolder(null).getTreeElement();
  ...
  /**
   * If AST has been gced and recreated, but someone still holds a reference to a PSI, then { #getPsi()} should return the very same PSI object.
   * So we try to find that PSI in file's { AstPathPsiMap}.
   */
  @Nullable
  ...
  protected PsiElement createPsiNoLock() {
    ...
    final ParserDefinition parserDefinition = (ParserDefinition) getParserDefinition(lang);
  ...
  private static void repairRemovedElement(final CompositeElement oldParent, final TreeElement oldChild) {
    ...
    final FileElement treeElement = DummyHolderFactory.createHolder( null, false).getTreeElement();
