 ...
 * Instead of { PsiElement#getReferences()}, clients can call { #getReferences(PsiReferenceService.Hints)} and the implementation
 ...
 * ({ com.intellij.psi.PsiReferenceService.Hints#offsetInElement}) or that have no change of resolving to a particular target
 * ({ com.intellij.psi.PsiReferenceService.Hints#target}).
 *
 * @since 144.*
 * @author peter
 */
public interface HintedReferenceHost extends PsiElement {
  ...
  /**
   * Same as { PsiElement#getReferences()}, but the implementation may take hints into account and return only references that match these hints.
  ...
  /**
   * Normally in { PsiElement#findReferenceAt(int)}, all tree hierarchy is traversed bottom-up and each element is asked for references.
