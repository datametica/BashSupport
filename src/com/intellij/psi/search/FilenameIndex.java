...
public class FilenameIndex extends ScalarIndexExtension<String> {
  ...
  public static Collection<VirtualFile> getVirtualFilesByName(final String name, final GlobalSearchScope scope) {
  ...
  public static PsiFile[] getFilesByName(final String name, final GlobalSearchScope scope) {
    return (PsiFile[])getFilesByName(name, scope, false);
  }
  ...
  public static boolean processFilesByName(@NotNull final String name,
    ...
    return processFilesByName(name, includeDirs, true, processor, scope, idFilter);
  }
  ...
  public static PsiFileSystemItem[] getFilesByName(final String name,
                                         @NotNull final GlobalSearchScope scope,
                                         boolean includeDirs) {
    ...
    processFilesByName(name, includeDirs, processor, scope,  null);
   ...
   */
  @NotNull
  public static Collection<VirtualFile> getAllFilesByExt( @NotNull String ext) {
      return getAllFilesByExt(ext, GlobalSearchScope.allScope());
  }

  @NotNull
  public static Collection<VirtualFile> getAllFilesByExt(@NotNull String ext, @NotNull GlobalSearchScope searchScope) {
    int len = ext.length();
    ...
    for (String name : getAllFilenames()) {
      ...
      if (length > len && name.substring(length - len).equalsIgnoreCase(ext)) {
        files.addAll(getVirtualFilesByName(name, searchScope));
      }
    }
    return files;
  }
