...
public abstract class StubTreeLoader {
  ...
  @Nullable
  public abstract ObjectStubTree readOrBuild( final VirtualFile vFile, @Nullable final PsiFile psiFile);

  @Nullable
  public abstract ObjectStubTree readFromVFile(final VirtualFile vFile);
  ...
  public String getStubAstMismatchDiagnostics(@NotNull VirtualFile file,
    ...
    if (!(psiFile instanceof PsiCompiledElement)) {
      ...
      PsiFile fromText = PsiFileFactory.getInstance().createFileFromText(psiFile.getName(), psiFile.getFileType(), text);
    ...
    ObjectStubTree latestIndexedStub = readFromVFile(file);
    ...
    if (document != null) {
      ...
      msg += "; committed: " + PsiDocumentManager.getInstance().isCommitted(document);
    }
    return msg;
  }
