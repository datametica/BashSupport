 ...
 * { #isParsable(ASTNode, CharSequence, Language, Project)} is invoked, and if it's successful,
...
public class IReparseableElementType extends ILazyParseableElementType {
  ...
  /**
   * Allows to construct element types without registering them, as in { IElementType#IElementType(String, Language, boolean)}.
   */
  ...
  public boolean isParsable(CharSequence buffer, Language fileLanguage) {
    return false;
  }

  /**
   * The same as { this#isParsable(CharSequence, Language, Project)}
   ...
   * @return true if the content is valid, false if not
   */
  public boolean isParsable(@Nullable ASTNode parent, CharSequence buffer, Language fileLanguage) {
    return isParsable(buffer, fileLanguage);
  }
}
