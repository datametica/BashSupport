...
public class FileContentImpl extends UserDataHolderBase implements FileContent {
  ...
  public PsiFile createFileFromText(@NotNull CharSequence text) {
    ...
    final Language substitutedLanguage = LanguageSubstitutors.INSTANCE.substituteLanguage(language, file);
    return PsiFileFactory.getInstance().createFileFromText(getFileName(), substitutedLanguage, text, false, false, true, file);
  }
  ...
  public FileType getSubstitutedFileType() {
    return SubstitutedFileType.substituteFileType(myFile, myFileType);
  }
  ...
  public PsiFile getPsiFileForPsiDependentIndex() {
    ...
    PsiFile psi = null;
    if (document != null) {
      PsiDocumentManager psiDocumentManager = PsiDocumentManager.getInstance();
