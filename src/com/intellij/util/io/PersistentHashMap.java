...
public class PersistentHashMap<Key, Value> extends PersistentEnumeratorDelegate<Key> implements PersistentMap<Key, Value> {
   ...
   * E.g. Value can be Set of String and individual Strings can be appended with this method for particular key, when { #get()} will
  ...
  /**
   * Process all keys registered in the map. Note that keys which were removed after { #compact()} call will be processed as well. Use
   * { #processKeysWithExistingMapping(Processor)} to process only keys with existing mappings
   */
  @Override
