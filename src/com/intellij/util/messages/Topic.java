...
/**
 * Defines messaging endpoint within particular { MessageBus bus}.
 *
...
public class Topic<L> {
   ...
   * <ul>
   *   <li>
   *     publisher { MessageBus#syncPublisher(Topic) receives} object that IS-A target interface from the messaging infrastructure.
   *     It calls target method with the target arguments on it then (method of the interface returned by the current method);
   *   </li>
   *   <li>
   *     the same method is called on handlers of all { MessageBusConnection#subscribe(Topic, Object) subscribers} that
  ...
  /**
   * { MessageBus Message buses} may be organised into { MessageBus#getParent() hierarchies}. That allows to provide
