...
public abstract class VirtualFileVisitor<T> {
   ...
   *
   * @param file a file to visit.
   * @return {plain #CONTINUE} to proceed to file's children,<br/>
   *         {plain #SKIP_CHILDREN} to skip to file's next sibling,<br/>
   *         result of {plain #skipTo(VirtualFile)} to skip to given file's next sibling.
   */
  @NotNull
   ...
   * from {plain #visitFile(VirtualFile)} or {plain #visitFileEx(VirtualFile)} methods).
   *
  ...
  /**
   * By default, visitor uses ({plain VirtualFile#getChildren()}) to iterate over file's children.
   * You can override this method to implement another mechanism.
   *
   * @param file a virtual file to get children from.
   * @return children iterable, or null to use {plain VirtualFile#getChildren()}.
   */
  @Nullable
  ...
  /**
   * Stores the {@code value} to this visitor. The stored value can be retrieved later by calling the { #getCurrentValue()}.
   ...
   * and the { #getCurrentValue()} returns the previous value which was stored here before the { #setValueForChildren} call.
   */
