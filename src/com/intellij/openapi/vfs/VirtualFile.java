...
/**
 * Represents a file in <code>{ VirtualFileSystem}</code>. A particular file is represented by equal
 * <code>VirtualFile</code> instances for the entire lifetime of the IntelliJ IDEA process, unless the file
 * is deleted, in which case { #isValid()} will return <code>false</code>.
 * <p/>
 * VirtualFile instances are created on request, so there can be several instances corresponding to the same file.
 * All of them are equal, have the same hashCode and use shared storage for all related data, including user data (see { UserDataHolder}).
 * <p/>
...
public abstract class VirtualFile extends UserDataHolderBase implements ModificationTracker {
   ...
   * { VirtualFile} changes.
   *
   *  VirtualFileListener#propertyChanged
   *  VirtualFilePropertyEvent#getPropertyName
   */
  public static final String PROP_NAME = "name";

  /**
   * Used as a property name in the { VirtualFilePropertyEvent} fired when the encoding of a
   * { VirtualFile} changes.
   *
   *  VirtualFileListener#propertyChanged
   *  VirtualFilePropertyEvent#getPropertyName
  ...
  /**
   * Used as a property name in the { VirtualFilePropertyEvent} fired when a visibility of a
   * { VirtualFile} changes.
   *
   *  VirtualFileListener#propertyChanged
   *  VirtualFilePropertyEvent#getPropertyName
  ...
  /**
   * Used as a property name in the { VirtualFilePropertyEvent} fired when a symlink target of a
   * { VirtualFile} changes.
   *
   *  VirtualFileListener#propertyChanged
   *  VirtualFilePropertyEvent#getPropertyName
   ...
   * <code>{ VirtualFileSystem}</code>. Format of the path depends on the concrete file system.
   * For <code>{ com.intellij.openapi.vfs.LocalFileSystem}</code> it is an absolute file path with file separator characters
   ...
   *  VirtualFileSystem#extractPresentableUrl
   */
  @NotNull
   ...
   * If file is not valid only { #equals}, { #hashCode} and methods from
   ...
   *         When IDEA has no idea what the file type is (i.e. file type is not registered via { FileTypeRegistry}),
   *         it returns { com.intellij.openapi.fileTypes.FileTypes#UNKNOWN}
   */
  @SuppressWarnings("JavadocReference")
  @NotNull
  ...
  /**
   * The same as { #refresh(boolean, boolean)} but also runs <code>postRunnable</code>
  ...
  /** @deprecated use { VirtualFileSystem#isValidName(String)} (to be removed in IDEA 18) */
  @Deprecated
