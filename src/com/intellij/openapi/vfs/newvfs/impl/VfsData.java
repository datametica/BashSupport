 ...
 *
 * The data is stored per-id in blocks of { #SEGMENT_SIZE}. File ids in one project tend to cluster together,
 ...
 *
 * 1. The file has not been instantiated yet, so { #getFileById} returns null.
 *
 ...
 * and creates the file instance. See { #initFile}
 *
 * 3. After that the file is live, an object representing it can be retrieved any time from its parent. File system roots are
 * kept on hard references in { com.intellij.openapi.vfs.newvfs.persistent.PersistentFS}
 *
 ...
 * all the listener have been notified about the file deletion and have had their chance to look at the data the last time. See { #killInvalidatedFiles()}
 *
 * 5. The file with removed data is marked as "dead" (see { #ourDeadMarker}, any access to it will throw { InvalidVirtualFileAccessException}
