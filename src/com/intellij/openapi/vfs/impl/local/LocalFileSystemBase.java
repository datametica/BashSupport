...
public abstract class LocalFileSystemBase extends LocalFileSystem {
  ...
  public VirtualFile createChildDirectory(Object requestor, @NotNull final VirtualFile parent, @NotNull final String dir) throws IOException {
    if (!isValidName(dir)) {
      throw new IOException(VfsBundle.message("directory.invalid.name.error".toString(), dir));
    }

    if (!parent.exists() || !parent.isDirectory()) {
      throw new IOException(VfsBundle.message("vfs.target.not.directory.error".toString(), parent.getPath()));
    }
    if (parent.findChild(dir) != null) {
      throw new IOException(VfsBundle.message("vfs.target.already.exists.error".toString(), parent.getPath() + "/" + dir));
    }

    File ioParent = convertToIOFile(parent);
    if (!ioParent.isDirectory()) {
      throw new IOException(VfsBundle.message("target.not.directory.error".toString(), ioParent.getPath()));
    }

    if (!auxCreateDirectory(parent, dir)) {
      File ioDir = new File(ioParent, dir);
      if (!(ioDir.mkdirs() || ioDir.isDirectory())) {
        throw new IOException(VfsBundle.message("new.directory.failed.error".toString(), ioDir.getPath()));
      }
    }
  ...
  public VirtualFile createChildFile(Object requestor, @NotNull final VirtualFile parent, @NotNull final String file) throws IOException {
    if (!isValidName(file)) {
      throw new IOException(VfsBundle.message("file.invalid.name.error".toString(), file));
    }
    ...
    if (parent.findChild(file) != null) {
      throw new IOException(VfsBundle.message("vfs.target.already.exists.error".toString(), parent.getPath() + "/" + file));
    }
    ...
    if (!auxCreateFile(parent, file)) {
      ...
      if (!FileUtil.createIfDoesntExist(ioFile)) {
        throw new IOException(VfsBundle.message("new.file.failed.error".toString(), ioFile.getPath()));
      }
    }
  ...
  public void deleteFile(Object requestor, @NotNull final VirtualFile file) throws IOException {
    if (file.getParent() == null) {
      throw new IOException(VfsBundle.message("cannot.delete.root.directory".toString(), file.getPath()));
    }

    if (!auxDelete(file)) {
      File ioFile = convertToIOFile(file);
      if (!FileUtil.delete(ioFile)) {
        throw new IOException(VfsBundle.message("delete.failed.error".toString(), ioFile.getPath()));
      }
    }
  ...
  public void moveFile(Object requestor, @NotNull final VirtualFile file, @NotNull final VirtualFile newParent) throws IOException {
    ...
    if (!file.exists()) {
      throw new IOException(VfsBundle.message("vfs.file.not.exist.error".toString(), file.getPath()));
    }
    if (file.getParent() == null) {
      throw new IOException(VfsBundle.message("cannot.rename.root.directory".toString(), file.getPath()));
    }
    if (!newParent.exists() || !newParent.isDirectory()) {
      throw new IOException(VfsBundle.message("vfs.target.not.directory.error".toString(), newParent.getPath()));
    }
    if (newParent.findChild(name) != null) {
      throw new IOException(VfsBundle.message("vfs.target.already.exists.error".toString(), newParent.getPath() + "/" + name));
    }

    File ioFile = convertToIOFile(file);
    if (FileSystemUtil.getAttributes(ioFile) == null) {
      throw new FileNotFoundException(VfsBundle.message("file.not.exist.error".toString(), ioFile.getPath()));
    }
    ...
    if (ioTarget.exists()) {
      throw new IOException(VfsBundle.message("target.already.exists.error".toString(), ioTarget.getPath()));
    }

    if (!auxMove(file, newParent)) {
      if (!ioFile.renameTo(ioTarget)) {
        throw new IOException(VfsBundle.message("move.failed.error".toString(), ioFile.getPath(), ioParent.getPath()));
      }
    }
  ...
  public void renameFile(Object requestor, @NotNull final VirtualFile file, @NotNull final String newName) throws IOException {
    if (!isValidName(newName)) {
      throw new IOException(VfsBundle.message("file.invalid.name.error".toString(), newName));
    }
    ...
    if (!sameName && parent.findChild(newName) != null) {
      throw new IOException(VfsBundle.message("vfs.target.already.exists.error".toString(), parent.getPath() + "/" + newName));
    }
    ...
    if (!auxRename(file, newName)) {
      if (!FileUtil.rename(ioFile, newName)) {
        throw new IOException(VfsBundle.message("rename.failed.error".toString(), ioFile.getPath(), newName));
      }
    }
  ...
  public VirtualFile copyFile(Object requestor,
    ...
    if (!isValidName(copyName)) {
      throw new IOException(VfsBundle.message("file.invalid.name.error".toString(), copyName));
    }
    ...
    if (newParent.findChild(copyName) != null) {
      throw new IOException(VfsBundle.message("vfs.target.already.exists.error".toString(), newParent.getPath() + "/" + copyName));
    }

    FileAttributes attributes = getAttributes(file);
    if (attributes == null) {
      throw new FileNotFoundException(VfsBundle.message("file.not.exist.error".toString(), file.getPath()));
    }
